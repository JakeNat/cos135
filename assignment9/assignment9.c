...
int queuePrint(int queue[], int currentlyQueued, int queueSize);
...
int main(int argc, char *argv[]) {
	...
	while (quitFactor != 1) {
		...
		// If user inputs "print"
			...
			queuePrint(queue,currentlyQueued,queueSize);
		}
		// If user inputs "quit"
...
int push(int toPush, int queue[], int queueSize) {
	...
	// If the queue is initialized to 0, can't push
	if (queueSize == 0) {
		printf("Overflow: %d\n",toPush);
	}
...
int queuePrint(int queue[], int currentlyQueued, int queueSize) {
	...
	// If the queue is initialized to 0, nothing to print
	if (queueSize == 0) {
		printf("\n");
		return 0;
	}
